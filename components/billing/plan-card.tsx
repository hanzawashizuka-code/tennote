...
export function PlanCard({ planKey, currentPlan, referralCode }: PlanCardProps) {
  ...
  return (
    <Card variant={planKey === "premium" ? "strong" : "default"} className="relative">
      {planKey === "premium" && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span className="bg-[#C8F400] text-[#0E1100] text-xs font-bold px-3 py-1 rounded-full">おすすめ</span>
        </div>
      )}
      <div className="mb-4">
        ...
        <div className="mt-1">
          {plan.priceJPY === 0 ? (
            <span className="text-2xl font-bold text-gray-900">無料</span>
          ) : (
            <>
              <span className="text-2xl font-bold text-gray-900">{formatJPY(plan.priceJPY)}</span>
              <span className="text-gray-400 text-sm">/月</span>
            </>
          )}
          {referralCode && isUpgrade && (
            <p className="text-[#C8F400] text-xs mt-1">初月10%割引適用</p>
          )}
        </div>
      </div>
      <ul className="flex flex-col gap-2 mb-5">
        {plan.features.map((f) => (
          <li key={f} className="flex items-center gap-2 text-sm text-gray-700">
            <Check size={14} className="text-[#C8F400] flex-shrink-0" />
            {f}
          </li>
        ))}
      </ul>
      {isCurrent ? (
        ...
        <Button variant="ghost" disabled className="w-full">ダウングレード</Button>
      ) : (
